...
class RiskEngine:
    def calculate_risk(self, indicator_type: str, vt_data: Dict[str, Any],
                       abuse_data: Dict[str, Any] | None = None) -> Dict[str, Any]:
        ...
        if vt_data.get("status") in SUCCESS_STATUSES:
            ...
            components["virustotal"] = {"score": vt_score, "weight": 50 if indicator_type == "ip" else 100, "source": "VirusTotal", "retrieved_at": vt_data.get("retrieved_at")}

        if indicator_type == "ip" and abuse_data.get("status") in SUCCESS_STATUSES:
            components["abuseipdb"] = {"score": int(abuse_data.get("abuseConfidenceScore") or 0), "weight": 50, "source": "AbuseIPDB", "retrieved_at": abuse_data.get("retrieved_at")}
        ...
        expected = 2 if indicator_type == "ip" else 1
